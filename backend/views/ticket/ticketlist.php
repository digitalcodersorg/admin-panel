...
<div class="post-index">
    ...
    <div class="alert btn-area">
        <form class="search-form" action="/support/tickets">
...
<?php $rr = isset($q['reponse']) ? $q['reponse'] : '' ?>
            ...
            <?php if ($role == 'admin') { ?>
            <?= Html::a(Yii::t('app', 'Download<i class="fa fa-arrow-down" aria-hidden="true"></i>'), ['ticket/download'], ['class' => 'btn btn-primary btn-sm download-ticket']); ?>
            <?php } ?>
        ...
        <table class="table table-striped jambo_table bulk_action ticket_list">
            <thead>
                <tr class="headings">
                    ...
                    <th class="column-title" style="width: 94px;">Last Update</th>
...
<?php foreach ($tickets as $ticket) { ?>
                    <tr id="ticket_<?= $ticket['ticket_code'] ?>" data-code="<?= $ticket['ticket_code'] ?>" data-url="<?= Url::to(['ticket/view', 'id' => $ticket['ID']]) ?>" data-action="<?= ($ticket['assigned_to_name'] == "") ? "assign_ticket" : 'view-ticket'; ?>" data-role="<?= $role ?>" class="view-ticket <?= ($ticket['notify'] == 'unseen') ? 'response' : '' ?>" style="cursor:pointer">
                        ...
                        <?php
                        $user = new User();
                        $company = $user->getUserMeta($ticket['ticket_owener'], "Company Name");
                        ?>
                        <td><?= mb_strimwidth($company, 0, 10, '...') ?></td>
                        ...
                        <td id="deart_<?= $ticket['ticket_code'] ?>">
                                <?php if(!empty($ticket['department_name'])){ ?>
                            <p class="ticket-subject"><strong>
                                <?php 
                                    $dep = new Department();
                                    $data = $dep->findBranch($ticket['department_id']);
                                    echo (isset($data[0]['name']) ? $data[0]['name'] : "N/A");
                                ?>
                                    </strong></p>
                                 <?= mb_strimwidth($ticket['department_name'], 0, 20, '..') ?>   
                                <?php }else{?>
                                    N/A
                                <?php }?>
